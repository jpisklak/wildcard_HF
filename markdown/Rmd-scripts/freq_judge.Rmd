...
include_graphics(
  file.path(main_dir,"plots/freq-judge/plt_fj_means.png")
)
```
...
**No Extreme vs. Extreme First | Value (High vs. Low):**

$b = `r b_vals[5]`$, $t(`r df_vals[5]`) = `r t_stat[5]`$, $p = `r p_vals[5]`$, $r = `r r_vals[5]`$

**Extreme Last vs. Extreme First | Value (High vs. Low):**

$b = `r b_vals[6]`$, $t(`r df_vals[6]`) = `r t_stat[6]`$, $p = `r p_vals[6]`$, $r = `r r_vals[6]`$

- IMPORTANT: p-values were adjusted for a one-sided hypothesis only for the first two planned contrasts. Since the interaction had no predicted direction, they were left as two-sided.
...
- ANOVA was conducted only on judgements for the risky extreme values (+20 and +80).
