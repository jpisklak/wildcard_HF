...
- Note: p-values for planned contrasts reflect a one-sided hypothesis test as per the pre-registered hypotheses.
...
```{r}
t_stat <- round(welch_E2_v_NE$statistic, 2)
df <- round(welch_E2_v_NE$parameter, 2)
p_val <- round(welch_E2_v_NE$p.value, 3)
d_val <- round(d_E2_v_NE$estimate, 2)
```
