...
source('R Scripts/fo_recall_plots.R')

#Create category column for results
fo <- fo %>%
  mutate(FO_eval = case_when(FO_stim == 'LR' & FO_resp == 20 ~ 'Yes',
...
fo_stats <- data.frame(
  ...
  Statistic = c(g1_test$statistic, 
                g2_test$statistic, 
                g3_test$statistic),
  
  df = c(g1_test$parameter, 
         g2_test$parameter, 
         g3_test$parameter),
  
  `p-value` = c(g1_test$p.value, 
                g2_test$p.value, 
                g3_test$p.value),
  
  OR = c(g1_effect$Global.statistics[1, 3],
         g2_effect$Global.statistics[1, 3],
         g3_effect$Global.statistics[1, 3]),
  
  Cohen_g = c(g1_effect$Global.statistics[3, 3],
              g2_effect$Global.statistics[3, 3],
              g3_effect$Global.statistics[3, 3]),
  
  lower_g_ci = c(g1_effect$Global.statistics[3, 4],
                 g2_effect$Global.statistics[3, 4],
                 g3_effect$Global.statistics[3, 4]),
  
  upper_g_ci = c(g1_effect$Global.statistics[3, 5],
