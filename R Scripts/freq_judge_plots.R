...
fj <- filter(data, phase == 'Freq_judge',
...
levels(fj$condition) <- c('Extreme 1st',
                          'Extreme Last',
                          'No Extreme')

#Create context column
fj <- fj %>%
  mutate(FJ_context = case_when(FJ_stim %in% c('LF', 'LR') ~ 'Low',
...
plt_fj_means <- ggplot(fj_long, aes(x = FJ_outcome, y = FJ_resp, 
