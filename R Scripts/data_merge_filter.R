...
#Unique ID
  #This is necessary because some incomplete files contain the same PROLIFIC_PID
  # and SESSION_ID as completed files.
...
data <- merge(data, df_sizes[, 2:3], by = "ID_unique",
      all.x = TRUE)
...
#WildCard
  #Note: Wildcard info was spread across 4 different columns depending on condition
...
data <- data %>%
  mutate(WC1 = case_when(Condition_2 == 0 ~ Extreme.Wild.Card,
                         Condition_2 == 1 ~ Extreme.Wild.Card,
                         Condition_2 == 2 ~ NonExtreme.Wild.Card.1
                         ))

data <- data %>%
  mutate(WC2 = case_when(Condition_2 == 0 ~ NonExtreme.Wild.Card,
...
data <- data %>%
  mutate(left_alt = case_when(LF == Left.Option.Image ~ 'LF',
...
data <- data %>%
  mutate(right_alt = case_when(LF == Right.Option.Image ~ 'LF',
...
#Response column
data <- data %>%
  mutate(response = case_when(Chosen.Door == 'Door1' ~ 'LF',
...
data <- merge(data, trials, by = "ID_unique",
              all.x = TRUE)
data <- data[order(data$abs_order), ]
...
data <- data %>%
  mutate(
    phase = case_when(Block >= 0 ~ 'choice',
...
#First outcome info
data <- data %>%
  mutate(FO_stim = case_when(Shown.Door..First.Outcome. == 'Door1' ~ 'LF',
...
data <- data %>%
  mutate(FJ_stim = case_when(Shown.Door == 'Door1' ~ 'LF',
                             Shown.Door == 'Door2' ~ 'LR',
                             Shown.Door == 'Door3' ~ 'HF',
                             Shown.Door == 'Door4' ~ 'HR',
...
data <- merge(data, demog, by = "PROLIFIC_PID",
              all.x = TRUE)
data <- data[order(data$abs_order), ]
...
if (length(cr_files) > 0) {file.remove(cr_files)}
